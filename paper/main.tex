...
The simulated GPU (\code{gpu.py}) extends the accelerator model with a parallel
processing hierarchy.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{gpu_hierarchy.pdf}
\caption{GPU processing hierarchy --- individual Processing Elements grouped into Workgroups, coordinated by the Ternary GPU controller}
\label{fig:gpuhier}
\end{figure}
...
\begin{description}
    \item[\textbf{Processing Element (PE)}]: The smallest compute unit, executing a
          single kernel thread with local register state. Each PE receives its global
          and local ID.
    \item[\textbf{Workgroup}]: A group of PEs sharing local memory and synchronization
          barriers. Workgroups enable cooperative computation on shared data.
    \item[\textbf{TernaryGPU}]: The complete hierarchy with configurable
          $\text{PEs\_per\_wg} \times n\_\text{workgroups}$ processing elements.
\end{description}

\section{Kernel Dispatch}

Kernels are Python callables distributed across all PEs. The dispatch mechanism:
\begin{enumerate}
    \item Creates the PE grid based on configuration;
    \item Assigns each PE its global and local indices;
    \item Executes the kernel on each PE (or each workgroup for cooperative kernels);
    \item Synchronizes workgroups at barrier points;
    \item Collects results.
\end{enumerate}

\section{Parallel Matrix Multiplication}

Matrix multiplication is decomposed into workgroups, each computing a tile of the
result matrix. PEs within a workgroup collaborate on tile computation with barrier
synchronization. This demonstrates the classic tiled matmul algorithm adapted for
ternary arithmetic.

The GPU mode is validated by 19 tests including kernel dispatch, tensor pipeline
execution, and visualization rendering.

\section{Visualization}

ASCII rendering tools in \code{viz.py}:
\begin{verbatim}
render_simd_lanes(trit_values)    → SIMD lane activity bar
render_tensor_matrix(data)        → 2D tensor content
render_matmul(A, B, result)       → Matrix multiplication stages
render_gpu_state(gpu)             → Full GPU state overview
render_packed_trits(data)         → Packed storage layout
\end{verbatim}
...
\code{test\_gpu.py} & GPU mode, kernel dispatch, visualization & 19 \\
