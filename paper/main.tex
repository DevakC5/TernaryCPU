...
characteristics of the Trinary system across 36 chapters with full instruction-set
...
\begin{itemize}
    \item \textbf{Part I -- Foundations}: Chapters~\ref{ch:intro}--\ref{ch:memory}
          ...
          via decimal round-trip, the arithmetic logic unit (ALU), register file, memory
...
\begin{enumerate}
    ...
    \item \textbf{CPU Layer}: Fetch-decode-execute cycle with 27 opcodes, 4 general-
          ...
          CALL/RET), interrupt system, and timer. Implemented in \code{cpu.py}.
...
\[
\code{add}(a,b) = \code{decimal\_to\_ternary}\bigl(
\code{ternary\_to\_decimal}(a) + \code{ternary\_to\_decimal}(b)\bigr).
\]
...
The \code{arithmetic.py} module provides four arithmetic operations, all using the
...
\subsection*{Addition}
\[
\code{add\_ternary}(a,b) = d2t\bigl(t2d(a) + t2d(b)\bigr)
\]

\subsection*{Subtraction}
\[
\code{subtract\_ternary}(a,b) = d2t\bigl(t2d(a) - t2d(b)\bigr)
\]
Handles negative results through signed-magnitude encoding.

\subsection*{Multiplication}
\[
\code{multiply\_ternary}(a,b) = d2t\bigl(t2d(a) \times t2d(b)\bigr)
\]
Supports negative operands via sign manipulation on the converted integers.

\subsection*{Division}
\[
\code{divide\_ternary}(a,b) = d2t\bigl(t2d(a) \mathbin{//} t2d(b)\bigr)
\]
Uses floor division. Raises \code{ZeroDivisionError} on division by zero.

The \code{validate\_ternary} lambda function ensures all inputs are well-formed before
computation.
...
Context & Path \\
\midrule
CPU ALU (ADD/SUB/MUL/DIV) & \code{arithmetic.py} (decimal round-trip) \\
Educational benchmarks & \code{adder.py} (native base-3) \\
PUSH/POP & Memory access (no arithmetic) \\
\bottomrule
\end{tabular}
\end{table}

\section{Formal Correctness of Decimal Round-Trip}

We prove that the decimal round-trip implements the same function as
native base-3 addition for the restricted domain of non-negative integers.
...
\[
\text{val}(s) = \sum_{i=0}^{n-1} d_i \cdot 3^i, \qquad
\text{str}_n(x) = \text{the unique } s \in \Sigma^n \text{ such that } \text{val}(s) = x.
\]

\noindent where $\text{str}_n(x)$ pads with leading zeros to length $n$.

\subsection{Theorem}

For all non-negative integers $a,b < 3^n$:

\[
\text{add\_ternary}(\text{str}_n(a),\; \text{str}_n(b)) = \text{str}_{n+1}(a + b).
\]

\noindent where the right-hand side may produce an $(n+1)$-trit result
representing values up to $2 \cdot 3^n - 2$.
...
\begin{enumerate}
    \item \textbf{Conversion}: $t2d(s) = \text{val}(s)$. Each digit
          contributes $d_i \cdot 3^i$. This matches the standard
          positional interpretation of a base-3 numeral.
    \item \textbf{Integer addition}: $a_{\text{int}} + b_{\text{int}} =
          \text{val}(s_a) + \text{val}(s_b)$. Integer addition over
          $\mathbb{Z}$ is associative, commutative, and closed for
          the given domain.
    \item \textbf{Reverse conversion}: $d2t(x)$ repeatedly divides
          $x$ by 3, recording remainders as ternary digits. This
          recovers $\text{str}(x)$ by the division algorithm:
          for any $x \in \mathbb{N}$, $x = q \cdot 3 + r$ with
          $r \in \{0,1,2\}$, uniquely determining the least
          significant digit. Iterating recovers all digits.
\end{enumerate}

\noindent Therefore $\text{add\_ternary}(s_a, s_b) = \text{str}(\text{val}(s_a)
+ \text{val}(s_b))$, which is exactly the positional base-3 sum.

\subsection{Extension to Signed Integers}

For signed-magnitude representation (leading $-$\code{)}:
\[
\text{val}(-s) = -\text{val}(s).
\]
The conversion functions handle the leading minus sign by negating the
integer value. Addition then follows the same round-trip path:
\[
\text{add\_ternary}(-a, b) = d2t\bigl(-t2d(a) + t2d(b)\bigr).
\]
The result inherits the correct sign through signed-magnitude encoding.
The same reasoning applies to subtraction, multiplication, and division,
each using the corresponding integer operation in step 2.
...
the domain is $0 \leq a,b < 3^{16} \approx 43$ million. The implementation
...
(\code{call\_stack}) entirely independent from the memory stack used by PUSH/POP.
...
\begin{description}
    \item[\code{CALL addr}] (format D, 3 cycles):
          \begin{enumerate}
              \item Push $\code{PC} + 1$ onto \code{call\_stack};
              \item Set $\code{PC} \gets \code{addr}$.
          \end{enumerate}
    \item[\code{RET}] (format E, 3 cycles):
          \begin{enumerate}
              \item Pop address from \code{call\_stack};
              \item Set $\code{PC} \gets \code{popped\_addr}$.
          \end{enumerate}
\end{description}
...
\begin{itemize}
    \item The \code{call\_stack} contains only return addresses;
...
RET on an empty call stack raises \code{StackUnderflowError}.
...
\begin{itemize}
    ...
    \item Return address is preserved on the \code{call\_stack} automatically.
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{assembler_flow.pdf}
...
The \code{ternary\_to\_int} and \code{int\_to\_ternary} conversion pair is the
...
a 256-byte instruction cache (\code{icache}) and a 256-byte data cache
(\code{dcache}).
...
\section{Direct-Mapped Organisation}

Both caches use a direct-mapped design, meaning each memory address maps
to exactly one cache line:

\[
\text{index} = \left\lfloor\frac{\text{address}}{\text{line\_size}}\right\rfloor
\bmod \text{num\_lines},\qquad
\text{tag} = \left\lfloor\frac{\text{address}}{\text{num\_lines} \times
\text{line\_size}}\right\rfloor.
\]

\subsection{CacheLine Structure}

Each \code{CacheLine} stores: \code{tag} (default $-1$),
...
Because each address maps to exactly one line in a direct-mapped cache,
no replacement-policy decision is required. On a miss, the line at the
computed index is evicted (with write-back if dirty), then the new
line is allocated. This design was chosen for deterministic timing,
lower hardware cost (no LRU state bits), and adequate hit rates
($>$95\%) for the demo-program workload.
...
\begin{description}
    \item[\code{flush\_line(address)}]: If the line at the address's
          index is dirty, write back and invalidate. If clean, simply
          invalidate.
    \item[\code{flush\_all()}]: Iterate all lines; dirty lines are
...
\code{address}, \code{data}, \code{is\_write} flag, and
\code{priority} (integer 0--3, higher = more urgent).
...
ordering. One transfer executes per cycle.
...
\begin{enumerate}
    ...
    \item \textbf{Service}: CPU flushes pipeline, reads IVT, sets PC to
          handler, injects 2-cycle context-switch cost.
    \item \textbf{EOI}: \code{IRET} calls \code{eoi()}, decrementing
          nesting depth. At depth 0, new interrupts can be acknowledged.
\end{enumerate}

\subsection{Nested Interrupts}

When an ISR is active and a higher-priority interrupt fires, the
controller acknowledges it. The programmer must preserve R0--R3 and
flags in the ISR prologue --- automatic context save/restore is not
implemented.
...
The \code{TernaryHillClimber} is a gradient-free evolutionary optimizer
that works on both \code{Perceptron} and
\code{TernaryNeuralNetwork}:
...
if no improvement occurs after \code{max\_attempts} (default 30)
...
\begin{description}
    \item[\code{\_train\_epoch\_sgd}]: For \code{SGDOptimizer}.
          ...
          step size.
    \item[\code{\_train\_epoch\_hillclimb}]: For
          \code{TernaryHillClimber}. Evaluates a fixed number of
...
\code{test\_optimizers.py} & 8 & SGD, hill-climber, AND/OR convergence \\
...
\begin{itemize}
    ...
    \item \textbf{CPU architecture}: A 27-opcode instruction set with 4
          general-purpose registers, dual stacks, a timer, an interrupt
          system, and a 5-stage pipeline with hazard detection and
    ...
    \item \textbf{Hardware simulation}: Cycle-accurate microarchitecture
          including pipeline, direct-mapped L1 cache, 2-bit branch
    ...
    \item \textbf{Artificial intelligence}: A complete ternary neural
          ...
          networks, SGD and hill-climber optimisers, eight datasets,
...
\subsection{Ternary Arithmetic via Decimal Round-Trip}

The decimal round-trip (ternary $\rightarrow$ integer $\rightarrow$
operation $\rightarrow$ ternary) proved to be the most practical
arithmetic implementation strategy. It handles negative numbers,
overflow, and arbitrary precision without the complexity of native
base-3 ripple-carry logic, at the cost of a small performance overhead
(absorbed by the native C backend).
...
Ternary neural networks using sign-sign updates and hill-climber
optimisation converge reliably on linearly separable functions and
can learn XOR through multi-layer architectures operating entirely
in the discrete ternary value space without floating-point gradients.
